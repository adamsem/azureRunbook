...
Param(
    ...
    [Parameter(Mandatory=$true)][ValidateNotNullOrEmpty()][String]$internal_automationaccountname,
    [Parameter(Mandatory=$true)][ValidateNotNullOrEmpty()][String]$internal_resourcegroupname,
    [Parameter(Mandatory=$true)][ValidateNotNullOrEmpty()][String]$external_waittimeforvmretryinseconds,
    [Parameter(Mandatory=$true)][ValidateNotNullOrEmpty()][String]$External_Start_ResourceGroupNames,
    [Parameter(Mandatory=$true)][ValidateNotNullOrEmpty()][String]$External_Stop_ResourceGroupNames,
    [Parameter(Mandatory=$true)][ValidateNotNullOrEmpty()][String]$External_ExcludeVMNames
)
...
function PerformActionOnSequencedTaggedVMAll($Sequences, [string]$Action, $TagName, $ExcludeList)
{
    foreach($seq in $Sequences)
    {
        ...
        if($WhatIf -eq $false)
        {
            ...
            {
                $AzureVMList += Get-AzResource -TagValue $seq | Where-Object {($_.ResourceType -eq “Microsoft.Compute/virtualMachines”) -and ($_.ResourceGroupName -eq $rg.ResourceGroupName)} | Select Name, ResourceGroupName
            }

            foreach($VM in $AzureVMList)
            {
                $FilterTagVMs = Get-AzVM -ResourceGroupName $VM.ResourceGroupName -Name $VM.Name
            ...
            if(($ExcludeList -ne $null) -and ($ExcludeList -ne "none"))
            {
                ...
                {
                    $currentVM = Get-AzVM | where Name -Like $filtervm.Trim()  -ErrorAction SilentlyContinue
...
function PerformActionOnSequencedTaggedVMRGs($Sequences, [string]$Action, $TagName, [string[]]$VMRGList, $ExcludeList)
{
    foreach($seq in $Sequences)
    {
        ...
        if($WhatIf -eq $false)
        {
            ...
            {
                $AzureVMList += Get-AzResource -TagValue $seq | Where-Object {($_.ResourceType -eq “Microsoft.Compute/virtualMachines”) -and ($_.ResourceGroupName -eq $rg.Trim())} | Select Name, ResourceGroupName
            }
...
function PerformActionOnSequencedTaggedVMList($Sequences, [string]$Action, $TagName, [string[]]$AzVMList)
{
    foreach($seq in $Sequences)
    {
        ...
        if($WhatIf -eq $false)
        {
            ...
            {
                $AzureVMList += Get-AzResource -TagValue $seq | Where-Object {($_.ResourceType -eq “Microsoft.Compute/virtualMachines”) -and ($_.Name -eq $vm.Trim())} | Select Name, ResourceGroupName
            }
        ...
        elseif($WhatIf -eq $true)
        {
            ...
            foreach($vm in $AzVMList)
            {
                $AzureVMList += Get-AzResource -TagValue $seq | Where-Object {($_.ResourceType -eq “Microsoft.Compute/virtualMachines”) -and ($_.Name -eq $vm)} | Select Name, ResourceGroupName
            }
...
$automationAccountName = $internal_automationaccountname
$aroResourceGroupName = $internal_resourcegroupname
$maxWaitTimeForVMRetryInSeconds = $external_waittimeforvmretryinseconds
$StartResourceGroupNames = $External_Start_ResourceGroupNames
$StopResourceGroupNames = $External_Stop_ResourceGroupNames
$ExcludeVMNames = $External_External_ExcludeVMNames
...
try
{
    ...
    $startTagKeys = Get-AzVM | Where-Object {$_.Tags.Keys -eq $startTagValue.ToLower()} | Select Tags
    $stopTagKeys = Get-AzVM | Where-Object {$_.Tags.Keys -eq $stopTagValue.ToLower()} | Select Tags
